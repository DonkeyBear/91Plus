...
export default class ChordSheetDocument {
  ...
  getKey() {
    const match = this.el.tkinfo?.textContent.match(/(?<=原調：)\w*/)
  ...
  getSinger() {
    const match = this.el.tinfo?.textContent.match(/(?<=演唱：).*(?=\n|$)/)
    return match ? match[0].trim() : ''
  }

  getComposer() {
    const match = this.el.tinfo?.textContent.match(/(?<=曲：).*?(?=詞：|$)/)
    return match ? match[0].trim() : ''
  }

  getLyricist() {
    const match = this.el.tinfo?.textContent.match(/(?<=詞：).*?(?=曲：|$)/)
    return match ? match[0].trim() : ''
  }
  ...
  getSheetText() {
    const formattedChordSheet = this.el.tone_z.textContent
      .replaceAll(/\s+?\n/g, '\n')
      .replaceAll('\n\n', '\n')
      .trim()
